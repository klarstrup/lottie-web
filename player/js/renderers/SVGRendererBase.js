...
SVGRendererBase.prototype.configAnimation = function (animData) {
  ...
  this.layerElement.setAttribute("clip-path", "url(#" + maskId + ")");
...
SVGRendererBase.prototype.destroy = function () {
  ...
  for (let i = 0; i < this.layers ? this.layers.length : 0; i += 1) this.elements[i]?.destroy();
...
SVGRendererBase.prototype.findIndexByInd = function (ind) {
  for (let i = 0; i < this.layers.length; i += 1) if (this.layers[i].ind === ind) return i;
...
SVGRendererBase.prototype.buildItem = function (pos) {
  ...
  if (this.layers[pos].tt) {
    const elementIndex =
      "tp" in this.layers[pos] ? this.findIndexByInd(this.layers[pos].tp) : pos - 1;
    if (elementIndex === -1) {
      return;
    }
