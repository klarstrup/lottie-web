...
function TransformProperty(elem, data, container) {
  ...
  if (data.p && data.p.s) {
    this.px = getProp(elem, data.p.x, 0, 0, this);
    this.py = getProp(elem, data.p.y, 0, 0, this);
    if (data.p.z) {
      this.pz = getProp(elem, data.p.z, 0, 0, this);
    }
  } else {
    this.p = getProp(elem, data.p || { k: [0, 0, 0] }, 1, 0, this);
  }
  if (data.rx) {
    this.rx = getProp(elem, data.rx, 0, degToRads, this);
    this.ry = getProp(elem, data.ry, 0, degToRads, this);
    this.rz = getProp(elem, data.rz, 0, degToRads, this);
    if (data.or.k[0].ti) {
      for (let i = 0; i < data.or.k.length; i += 1) {
        data.or.k[i].to = null;
        data.or.k[i].ti = null;
      }
    }
    this.or = getProp(elem, data.or, 1, degToRads, this);
    // sh Indicates it needs to be capped between -180 and 180
    this.or.sh = true;
  } else {
    this.r = getProp(elem, data.r || { k: 0 }, 0, degToRads, this);
  }
  if (data.sk) {
    this.sk = getProp(elem, data.sk, 0, degToRads, this);
    this.sa = getProp(elem, data.sa, 0, degToRads, this);
  }
  this.a = getProp(elem, data.a || { k: [0, 0, 0] }, 1, 0, this);
  this.s = getProp(elem, data.s || { k: [100, 100, 100] }, 1, 0.01, this);
  // Opacity is not part of the transform properties, that's why it won't use this.dynamicProperties. That way transforms won't get updated if opacity changes.
  if (data.o) {
    this.o = getProp(elem, data.o, 0, 0.01, elem);
  } else {
