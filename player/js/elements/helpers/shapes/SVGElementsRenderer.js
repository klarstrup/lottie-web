...
function renderPath(styleData, itemData, isFirstFrame) {
  ...
  for (let l = 0; l < itemData.styles.length; l += 1) {
    let mat;
    let iterations;
    let redraw = itemData.sh._mdf || isFirstFrame;
    if (itemData.styles[l].lvl < lvl) {
      mat = _matrixHelper.reset();
      iterations = lvl - itemData.styles[l].lvl;
      let k = itemData.transformers.length - 1;
      ...
      if (redraw) {
        ...
        while (iterations > 0) {
          const props = itemData.transformers[k].mProps.v.props;
          mat.transform(
            props[0],
            props[1],
            props[2],
    ...
    if (redraw) {
      const paths = itemData.sh.paths;
      pathStringTransformed = "";
      for (let j = 0; j < paths._length; j += 1) {
...
function renderGradient(styleData, itemData, isFirstFrame) {
  ...
  if (hasOpacity && (itemData.g._omdf || isFirstFrame)) {
    ...
    const stops = itemData.g._collapsable ? itemData.cst : itemData.ost;
...
export function createRenderFunction(data) {
  switch (data.ty) {
    case "fl":
      return renderFill;
    case "gf":
